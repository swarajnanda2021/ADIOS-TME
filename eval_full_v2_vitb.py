"""Path B-2 evaluation: ViTBCellViT on PanNuke Test.

Copy of ``eval_full_v1.py`` with two changes:
  * Model construction: ``ViTBCellViT`` built from a ViT-B encoder loaded
    via ``load_vitb_encoder``. No ADIOS prior — inference is encoder-only.
  * Argparse: ``--adios_checkpoint`` and ``--stage1_selector`` removed in
    favor of ``--vitb_checkpoint``.

Everything else (``__proc_np_hv``, ``aggregated_jaccard_index``, per-class
instance PQ, confusion-matrix classification, extras characterization) is
identical so the result table is directly comparable to the Path Z eval.

Writes:
- logs/eval/eval_report_v2_vitb.txt   (ASCII; cat-friendly)
- logs/eval/eval_results_v2_vitb.json (raw values)
"""
...
def parse_args():
    ...
    p.add_argument('--checkpoint', default='./logs/vitb_pathb2/vitb_pathb2.pth',
                   help='Trained ViTBCellViT state dict.')
...
def _load_vitb_config(config_path):
...
def main():
    ...
    config = _load_vitb_config(args.config)
    ...
    with torch.no_grad():
        for image, mask_2ch, distance_map, instance_mask, class_mask in test_loader:
            ...
            np_arr = output['masks'].cpu().numpy()
            hv_arr = output['distances'].cpu().numpy()
            nc_arr = output['nuclei_types'].argmax(1).cpu().numpy()
    ...
    L('=' * 78)
    L(f'  ViTBCellViT Evaluation Report (v2_vitb)')
    L('=' * 78)
    L('')
    ...
    report_text = '\n'.join(lines)
    report_path = os.path.join(args.output_dir, 'eval_report_v2_vitb.txt')
    ...
    json_path = os.path.join(args.output_dir, 'eval_results_v2_vitb.json')
