"""Submitit launcher for stage1 of the nuclei-counter pipeline."""
...
SHARED_LOG_DIR = Path(WORK_DIR) / "logs" / "stage1_submitit"
...
class Trainer:
    ...
    def __call__(self):
        ...
        cmd = ["python", "train_stage1_selector.py", "--config", "configs.nuclei_counter"]
...
def main():
    ...
    job_name = f"nuclei_stage1_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    executor.update_parameters(
        mem_gb=128, gpus_per_node=1, tasks_per_node=1, cpus_per_task=8, nodes=1,
        timeout_min=2880,
        slurm_partition="preemptable",
        slurm_signal_delay_s=120,
        slurm_gres="gpu:1",
        slurm_constraint="h100|a100",
